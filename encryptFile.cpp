...
void test_copy_hex(const char* inFile, const char* outFile)
{
	...
	std::ofstream os(outFile, std::ios::out | std::ios::binary);
...
{
	void encrypt_h(const char* iv, const char* key, const char* inputFile, const char* outputFile)
	{//encrypt using the hex form
	 //DO NOT USE, for the moment this one does not work properly
		CryptoPP::AES::Encryption aesEncryption((byte *)key, CryptoPP::AES::MAX_KEYLENGTH);
		CryptoPP::CBC_Mode_ExternalCipher::Encryption cbcEncryption(aesEncryption, (byte *)iv);
		std::ifstream ifs(inputFile, std::ios::binary | std::ios::in);
		std::ofstream os(outputFile, std::ios::binary | std::ios::out);
		ifs >> std::noskipws;
		...
		{
			CryptoPP::StreamTransformationFilter stfEncryptor(cbcEncryption, new CryptoPP::StringSink(ciphertext));
			stfEncryptor.Put(reinterpret_cast<const unsigned char*>(line), strlen(line));
			stfEncryptor.MessageEnd();
			for (int i = 0; i < ciphertext.length(); i++)
			{
				os << std::hex << ciphertext.at(i);
			}
		}
		string ciphertext;
